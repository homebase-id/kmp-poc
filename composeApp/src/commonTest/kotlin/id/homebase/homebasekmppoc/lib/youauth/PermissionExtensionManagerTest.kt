...
class PermissionExtensionManagerTest {
    ...
    private fun createSecurityContextJson(
        ...
        val permissionGroup = buildJsonObject {
            putJsonObject("permissionSet") {
                putJsonArray("keys") { permissions.forEach { add(JsonPrimitive(it)) } }
            }
            putJsonArray("driveGrants") {
                driveGrants.forEach { grant ->
                    add(Json.encodeToJsonElement(DriveGrant.serializer(), grant))
                }
            }
        }
    ...
    fun testGetMissingPermissions_missingPermission_returnsResult() = runTest {
        ...
        val jsonResponse =
                createSecurityContextJson(permissions = listOf(AppPermissionType.ManageFeed.value))
        ...
        assertTrue(
                result.extendPermissionUrl.contains("p=${AppPermissionType.ReadConnections.value}")
        )
    }
    ...
    fun testGetMissingPermissions_driveInsufficientPermission_returnsResult() = runTest {
        ...
        val jsonResponse = createSecurityContextJson(driveGrants = listOf(DriveGrant(grantedDrive)))
