...
)
{
    ...
    suspend fun sync(): Flow<BackendEvent> = flow {
        if (!mutex.tryLock()) {
            emit(BackendEvent.SyncUpdate.Failed(driveId,"Another sync already in progress"))
            return@flow
        }
        try {
            var totalCount = 0
            ...
            while (keepGoing)
            {
                ...
                val durationMs = measureTimedValue {
                    try {
                        ...
                        emit(BackendEvent.SyncUpdate.Failed(driveId,"Sync failed: ${e.message}"))
            ...
            emit(BackendEvent.SyncUpdate.Completed(driveId, totalCount))
        } finally {
            mutex.unlock()
        }
